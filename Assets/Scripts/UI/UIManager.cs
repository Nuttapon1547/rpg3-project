...
public class UIManager : MonoBehaviour
{
    ...
    public void SelectHeroByAvatar(int i)
    {
        ...
        {
            PartyManager.instance.SelectSimgleHeroByToggle(i);
        }
        else
        {
    ...
    public void ShowCharPanel()
    {
        ...
        string stat = string.Format("Level: {0}\nExperience: {1}\n" + "Attack Damage: {2}\nDefense Power: {3}", hero.Level, hero.Exp, hero.AttackDamage, hero.DefensePower);
        statText.text = stat;

        string ability = string.Format("Strength: {0}\nDexterity: {1}\n" + "Constitution: {2}\nIntelligence: {3}\n" + "Wisdom: {4}\nCharisma: {5}", hero.Strength, hero.Dexterity, hero.Constitution, hero.Intelligence, hero.Wisdom, hero.Charisma);
